                    ...
                    <div class="col-lg-12">
                          ...
                          <nav aria-label="breadcrumb" style="margin-top: 40px;">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><a href="s_order_confirm.php">Confirm <small><span class="badge badge-primary"> 
                                          <?php
                                         $con=mysqli_connect('localhost','root','','pick_a_plant');
                                ...
                                <li class="breadcrumb-item active" aria-current="page"><a href="s_order_cancel.php"><u>Cancelled
                                        ...
                                        <table id="employee_data" class="table table-striped table-bordered">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <td style="width: 100px;"><small><strong>Status</strong></small></td>
                                                    <td style="width: 20px;"><small><strong>Reason</strong></small></td>
                                            ...
                                            for ($i = 1; $i <= $rowcount; $i++) {
                                                ...
                                                if ($status == 2) {
                                                    $action = "Cancelled";
                                                    $diable1 = ' <button class="btn btn-danger discancel" disabled="">Cancelled</button>';

                                                    $order= ' <button class="btn btn-danger discancel" disabled="">Cancelled</button>';
                                                    $disable1= ' <button class="btn btn-danger discancel" disabled="">Cancelled</button>';
                                                    $reason='<i class="fa fa-exclamation-circle fa-lg" aria-hidden="true" style="color: #e48a89;"></i>';
                                            ...
                                            <a class="userinfo" style= "color:sky blue; cursor: pointer;" data-toggle="modal" 
                                            data-target="#' . $row1['id'] . 'issue">' . $reason . '</a></center>' . '</td> 
                                             </form> 
                                            </tr> 
...
<!-- 2ND MODAL FOR ISSUE LAST NAV TABLE -->
<!-- 2ND MODAL FOR ISSUE LAST NAV TABLE -->
<!-- 2ND MODAL FOR ISSUE LAST NAV TABLE -->
<!-- 2ND MODAL FOR ISSUE LAST NAV TABLE -->
<!-- 2ND MODAL FOR ISSUE LAST NAV TABLE -->
...
<div class="modal fade" role="dialog" id="<?php echo $row1['id']; ?>issue">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
...
<h5>DEAR Ma'am/Sir<br>
         <?php 
    $reason=$row1['reason'];
    $reason2=$row1['reason2'];

    if (empty($reason)) {
         $reason101='<h5><strong>The order han been cancelled due to  '.$row1['reason2'].' </strong></h5>';
    ...
    if (empty($reason2)) {
        $reason101='<h5><strong>The order han been cancelled due to '. $row1['reason'].' </strong></h5>';
    ...
    if (!empty($reason) and !empty($reason2)) {
        $reason101='<h5><strong>The order han been cancelled due to  '. $row1['reason'].' and '.$row1['reason2'].' </strong></h5>';
